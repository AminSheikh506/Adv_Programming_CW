...
HUGO PIPER:
  ...
  1) Create "Create Server" functionality via the CLI and inform the server creator that they have been assigned the role of "coordinator"
  
  2) Create "Join Server via IP and PORT functionality"
 
  3) Establish coordinator logic incase the server admin (ex-coordinator) leaves the server:
...
AMIN SHEIKH:
  ...
  1) Create a system where the client will be informed about who the current coordinator is via the CLI upon joining a server (Emon will add this to the GUI later)
  
  2) Create a system where the user can input their ID (username) before connecting to the server.
  ...
  3) Create system where clients can request to view all the members details from the coordniator via the CLI (emon will encorprate this into the GUI later):
      - ID's
      - IP ADDRESSES
      - PORTS
...
KIPP SUMMERS:
  ...
  1) Create multithreading system so the server can handle multiple clients with ease:
  ...
  2) Optimize code and ensure all systems are running efficiently and threads are encorprated properly within the code
 
  3) Conduct JUnit based testing and fault tolerance testing once the code and GUI is complete.


JAHID EMON:

1) Create the homepage GUI:
